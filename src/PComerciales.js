import React from "react";
import "../src/styles/Styles.scss"

const PComerciales=()=>{


    return(
        <div className="mainContainer">
            <h1 className="centeredText">Quienes somos</h1>
            <p>Compumax nació en el año 2021 a partir de un grupo de entusiastas de la informática, quienes aunaron su experiencia técnica, su pasión por la tecnología 
            y su capital a fin de poder brindarle a sus clientes un servicio único en el país.</p>
            <h2>Nuestra visión</h2>
            <p>Ser la mejor empresa de venta de informática on-line del país, donde se pueda adquirir desde una simple pieza de computadora hasta un sistema completo "llave en mano".</p>
            <h2>Nuestra misión</h2>
            <p>Ayudar a la gente a que trabajar y divertirse con las computadoras sea una experiencia fácil, placentera y altamente productiva.</p>
            <h2>Nuestros valores</h2>
            <p>Contamos con personal altamente capacitado para entender los problemas de quienes acuden por nuestros servicios, y de proveerles la mejor solución a su necesidad en términos 
            económicos y funcionales.</p>
            <h1>  </h1><br />
            <h1>  </h1><br />
            <h1>  </h1><br />
            <p>Compumaxx...maximiza tu experiencia!!!</p>
        </div>
    );
}

export default PComerciales
